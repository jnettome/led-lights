from flask import Flask, request, jsonify
import time
import math
from ledstrip import LEDStrip

app = Flask(__name__)

# GPIO pins for CLK and DAT
CLK = 13
DAT = 12

# Create an LEDStrip instance
strip = LEDStrip(CLK, DAT)

# Modes of operation
class Mode:
    STATIC_COLOR = 1
    COLOR_CYCLE = 2
    BREATHING = 3

# mode = Mode.STATIC_COLOR  # Initial mode
# print(f"Initial mode set to STATIC_COLOR.")
mode = Mode.BREATHING  # Initial mode
print(f"Initial mode set to BREATHING.")

# Color definitions
staticColor = (255, 20, 147)  # DeepPink
cycleColors = [(255, 87, 51), (227, 11, 92), (64, 224, 208)]
colorIndex = 0
colorChangeInterval = 5  # Interval in seconds
transitionTime = 1       # Transition time in seconds
lastColorChangeTime = time.time()

# Breathing mode
breathCycleDuration = 7  # Duration in seconds

def switchMode():
    global mode
    mode = (mode % 3) + 1
    mode_name = {1: "STATIC_COLOR", 2: "COLOR_CYCLE", 3: "BREATHING"}
    print(f"Mode switched to {mode_name[mode]}.")

def staticColorMode():
    print("Executing Static Color Mode.")
    print(f"Static color set to: {staticColor}.")
    strip.setcolourrgb(*staticColor)

def colorCycleMode():
    global colorIndex, lastColorChangeTime
    current_time = time.time()
    if current_time - lastColorChangeTime > colorChangeInterval:
        lastColorChangeTime = current_time
        colorIndex = (colorIndex + 1) % len(cycleColors)
        print(f"Color cycled to index {colorIndex}, color {cycleColors[colorIndex]}.")
        strip.setcolourrgb(*cycleColors[colorIndex])

def breathingMode():
    current_time = time.time()
    timeIntoCycle = (current_time % breathCycleDuration) / breathCycleDuration
    brightness = (math.sin(timeIntoCycle * 2 * math.pi) + 1) / 2
    adjustedColor = tuple(int(c * brightness) for c in cycleColors[0])
    print(f"Breathing mode: brightness {brightness:.2f}, color {adjustedColor}.")
    strip.setcolourrgb(*adjustedColor)

@app.route('/health.json', methods=['GET'])
def health():
    return jsonify({
        'mode': mode,
        'staticColor': f'#{staticColor[0]:02x}{staticColor[1]:02x}{staticColor[2]:02x}',
        'cycleColors': [f'#{color[0]:02x}{color[1]:02x}{color[2]:02x}' for color in cycleColors]
    })

@app.route('/change_mode.json', methods=['POST'])
def change_mode():
    global mode
    mode = (mode % 3) + 1
    return jsonify({'new_mode': mode})



def main_loop():
    try:
        while True:
            print(f"Current mode: {mode}")
            # if mode == Mode.STATIC_COLOR:
            #     strip.setcolourrgb(*staticColor)
            # elif mode == Mode.COLOR_CYCLE:
            #     strip.setcolourrgb(*cycleColors[colorIndex])
            # elif mode == Mode.BREATHING:
            #     adjustedColor = tuple(int(c * ((math.sin(time.time() * math.pi / 3.5) + 1) / 2)) for c in cycleColors[0])
            #     strip.setcolourrgb(*adjustedColor)
            if mode == Mode.STATIC_COLOR:
                staticColorMode()
            elif mode == Mode.COLOR_CYCLE:
                colorCycleMode()
            elif mode == Mode.BREATHING:
                breathingMode()
            time.sleep(1)
    except KeyboardInterrupt:
        print("Program exited gracefully")
    finally:
        strip.cleanup()

if __name__ == '__main__':
    # from threading import Thread
    # Thread(target=main_loop, daemon=True).start()
    # app.run(debug=True, host='0.0.0.0')
    main_loop()

